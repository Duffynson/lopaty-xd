...
require_once '../../php/db.php';
    if (isset($_POST['submit'])){
      ...
      if (!(empty($aktualnost) || empty($originalita) || empty($odbornost) || empty($jazyk) || empty($comment))){
        mysqli_query($conn, "UPDATE Recenze SET aktualnost = $aktualnost, originalita = $originalita, jazyk = $jazyk, odbornost = $odbornost, text = $comment, datum_recenze = $date WHERE ID_recenze = $_GET["id"]");
        mysqli_close($conn);
